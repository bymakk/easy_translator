...
**Минимум за один перевод — 300 токенов (≈ 0.30 ₽).** Такой минимум нужен потому, что каждый запрос к Grok включает не только сам текст, но и системный промпт плюс набор примеров, которые заставляют модель отвечать в нужном стиле и справляться со сленгом, идиомами и матом. Короткие переводы (одно слово, «ок», «thanks») всё равно стоят минимум 0.30 ₽ — платите вы по сути за качество модели, а не за количество букв.

**Что получается за 100 ₽ (= 100 000 токенов) при повседневном использовании:**
...
Для типичного повседневного сценария (читаем статьи/переписку, время от времени попадаются незнакомые слова и предложения) 100 ₽ хватает обычно на **несколько сотен переводов** — это недели и месяцы использования. Если AI-токены всё же закончились, расширение молча переключится на бесплатный Google Translate, ничего сломано не будет.
